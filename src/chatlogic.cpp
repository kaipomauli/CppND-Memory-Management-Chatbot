...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (childNode->get())->AddEdgeToParentNode(_edges.back());
                            (parentNode->get())->AddEdgeToChildNode(std::move(_edges.back()));
                        }
    ...
    ChatBot mychatbot("../images/robot.png");
    mychatbot.SetRootNode(rootNode);
    mychatbot.SetChatLogicHandle(this);
    mychatbot.SetCurrentNode(_currentNode);
    _chatBot = &mychatbot;
